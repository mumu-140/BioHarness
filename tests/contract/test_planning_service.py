...
from bioharness.application.plan_analysis import (
    AssessmentDependencyMismatch,
    PlanningService,
    assessment_dependency_fingerprint,
)
...
from tests.fakes.factories import make_data_ref, make_task
...
class PlanningSpy:
    ...
    def get_data_ref(self, value_id):
        return next((value for value in self.data_refs if value.id == value_id), None)

    def add_policy_decision(self, value):
        self.policy.append(value)

    def add_data_ref(self, value):
        self.data_refs.append(value)
    ...
    def add_run_spec(self, value):
        self.run_specs.append(value)


class SpyUow:
    def __init__(self, planning, order=None):
    ...
    def commit(self):
        ...
        if self.order is not None:
            self.order.append(("uow", "commit"))

    def rollback(self):
        pass


def uow_factory_for(planning, order=None):
    return lambda: SpyUow(planning, order=order)


def test_authorization_is_durable_before_provider_resolution():
    order = []
    task = make_task()
    ...
    service = ResolutionService(
        policy=policy,
        provider=provider,
        uow_factory=uow_factory_for(planning, order=order),
        clock=lambda: NOW,
    )
...
def test_authorization_remains_recorded_when_provider_resolution_fails():
    order = []
    task = make_task()
    planning = PlanningSpy(task)
    policy = FakePolicyEvaluator(order=order)
    ...
    class FailingProvider:
        def resolve(self, logical_resources, context):
            order.append(("provider", "resolve"))
...
def test_denied_resolution_does_not_touch_provider():
    ...
    service = ResolutionService(
        policy=policy,
        provider=provider,
        uow_factory=uow_factory_for(planning),
        clock=lambda: NOW,
    )
...
def test_analysis_identity_uses_resolved_content_not_database_uuid():
    task = make_task()
    ref_a = make_data_ref()
    ref_b = ref_a.model_copy(
        update={"id": UUID("00000000-0000-0000-0000-000000000199")}
    )

    def publish(ref):
        fingerprint = assessment_dependency_fingerprint(
            task=task,
            resolved_data_ref_ids=(ref.id,),
            contract_id="generic",
            contract_revision="1",
        ...
        planning = PlanningSpy(task)
        planning.data_refs.append(ref)
        service = PlanningService(
            uow_factory=uow_factory_for(planning), clock=lambda: NOW
        )
        return service.publish_run_spec(
            task=task,
            assessment=assessment,
            resolved_data_ref_ids=(ref.id,),
            provider_workflow_identity={"provider": "fake", "revision": "r1"},
    ...
    assert publish(ref_a).analysis_hash == publish(ref_b).analysis_hash
